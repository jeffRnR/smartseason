...
export class FieldService {
  ...
  async getFieldById(id: string, userId: string, userRole: Role): Promise<FieldWithStatus> {
    const field = await fieldRepository.findById(id);
    if (!field) throw new Error('Field not found');
    if (userRole === Role.AGENT && field.agentId !== userId) throw new Error('Access denied');
  ...
  async updateField(
    ...
    if (userRole === Role.AGENT && existing.agentId !== userId) throw new Error('Access denied');
    ...
    const newStage = (updateData.currentStage as Stage) ?? existing.currentStage;
  ...
  async getDashboardStats(userId: string, userRole: Role): Promise<DashboardStats> {
    ...
    const stats: DashboardStats = {
      ...
      atRiskFields: 0,
      completedFields: 0,
      fieldsByStage: { PLANTED: 0, GROWING: 0, READY: 0, HARVESTED: 0 },
    };
    fields.forEach((f) => {
